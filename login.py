...
root.geometry("400x400+200+100")  # 넓이 설정
root.resizable(False, False)  # 크기 조정 가능한지 x축, y축

MAX_LENGTH = 6

title = Label(root, text="CLOUD 로그인", font='Helvetica 20 bold')
...
emailFrame.pack()
lblEmail = Label(emailFrame, text="e-mail", width='8',font=("맑은 고딕", '9','bold'), anchor="w")  # 넓이 8, anchor='w'해서 왼쪽 정렬
...
passwordFrame.pack()
lblPassword = Label(passwordFrame, text="password", width='8',font=("맑은 고딕", '9', 'bold'), anchor="w")
...
def login():
    ...
    if (email.get() == dbemail[k]) and (password.get() == dbpassword) and (rand.get().upper() == tmp_str):
        new = Tk()
        new.title("successful login")
        new.geometry("400x400+200+100")
        new.resizable(False, False)
        welcome = Label(new, text="Welcome!!", width=20, height=25)
        welcome.pack()
        new.mainloop()
    else:
        count += 1
